...
.PARAMETER DatePreset
  Preset de fecha de Meta Ads: today, yesterday, last_3d, last_7d, last_14d, last_28d, last_30d, last_90d, this_week_mon_today, this_week_sun_today, last_week_mon_sun, last_week_sun_sat, this_month, last_month, this_quarter, maximum.
...
function Invoke-MetaGet {
  ...
  $baseUri = "https://graph.facebook.com/$GraphVersion/$Path"
  $qs = ConvertTo-QueryString -Params $Params
  $currentUri = if ($qs) { "$baseUri?$qs" } else { $baseUri }
