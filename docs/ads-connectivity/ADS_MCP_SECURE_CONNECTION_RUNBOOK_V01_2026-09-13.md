...
Rama: `docs/marketing-ads-mcp-connectivity-runbook`  
Estado: `DRAFT_VALIDATED_PARTIAL` — Google Ads READ validado hasta MCP local; unión final MCP real ↔ tunnel pendiente de cierre de #104.
...
TUNNEL_REAL_TO_GOOGLE_ADS_MCP=PENDING_ISSUE_104
ADS_WRITES=0
SECRETS_IN_GITHUB=0
```
...
Estado al escribir este runbook:

```text
PENDING_ISSUE_104
```

No fijar un comando definitivo hasta que Codex inspeccione la ayuda real de `tunnel-client.exe` y valide el launcher. El intento inicial fue inválido porque faltaba `CONTROL_PLANE_TUNNEL_ID` en la nueva ventana PowerShell.
...
- PR #103 — reporting bridge rechazado como solución primaria; no usar como arquitectura principal.
...
REAL_MCP_TUNNEL_INTEGRATION=PENDING
...
`NEXT_BEST_ACTION`: cerrar #104, incorporar al runbook el launcher final confirmado y luego usar este documento como baseline para Google Ads WRITE y Meta Ads READ/WRITE.
