...
| 2026-08-13 | [SUPERSEDED en arquitectura de almacenamiento por decisión 2026-09-05] SharePoint/OneDrive Empresa como bodega definitiva y external-files como staging local. | Fuentes pesadas / continuidad | Amarillo | `AGENTS.md`; SUPERSEDED |
...
| 2026-09-05 | Regla canónica de fuentes pesadas: SharePoint Site `Documentos/CAPACITA/Proyectos/external-files/marketing-performance-capacita` es la única bodega canónica definitiva de binarios pesados. OneDrive `Sitio de comunicación - external-files` es acceso sincronizado local, no segunda bodega ni staging. GitHub almacena únicamente documentación liviana, código, tests y fixtures sanitizados. | Fuentes pesadas / arquitectura canónica | Verde/Amarillo | `AGENTS.md`; `ESTANDAR_FUENTES_PESADAS_CAPACITA.md` |
| 2026-09-05 | Implementar Negative Keyword Guard e inventario de control plane oficial READ (Fase 0 y Fase 1 de Task Hub #215 / Issue #85). Rescate sanitizado de PR #52 en PR #86 (PR #52 histórico intacto sin mergear); negativas evaluadas con deduplicación, separación B2C/B2B fail-closed, idempotencia persistente entre procesos y `NEGATIVE_LIVE_SNAPSHOT=HOLD_DATA_GAP` ante ausencia de auth viva; `NEGATIVE_GUARD_OFFLINE_CORE=PASS`; `LIVE_NEGATIVE_ADAPTER=IMPLEMENTED_HOLD_AUTH`; cero writes en Ads/CRM/producción. | Control Plane READ y Guard de Negativas | Verde/Amarillo | Task Hub #215; Issue #85; PR #86 |
